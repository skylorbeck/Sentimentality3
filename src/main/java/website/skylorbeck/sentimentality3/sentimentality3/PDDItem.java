...
public class PDDItem extends Item {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        ...
        if(mode){//gets the light level for the block the item is in depending on the mode set in the NBT
            ...
            stack.setCustomName(Text.translatable("item.sentimentality3.pdd_day"));
        } else {
            light = world.getLightLevel(LightType.BLOCK,entity.getBlockPos());
            stack.setCustomName(Text.translatable("item.sentimentality3.pdd_block"));
        }
    ...
    public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        tooltip.add(Text.translatable("item.sentimentality3.pdd_tip"));
